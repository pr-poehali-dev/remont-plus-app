...
export default function OfficeSidebar({ zones, activeId, totalAll, markupPct, regionId, onSelectZone }: Props) {
  ...
  const laborBase =
    ...
    (az.blockStairs ? (STAIR_FINISH_OPTIONS.find(s => s.id === az.stairFinish)?.pricePerStep ?? 0) * az.stairFlights * az.stairStepsPerFlight + (az.stairRailing ? az.stairFlights * az.stairWidth * 12000 : 0) + (az.stairAntiSlip ? az.stairFlights * az.stairStepsPerFlight * 850 : 0) : 0);
  ...
  const breakdown = [
    ...
    { label: "Лестничные марши",     enabled: az.blockStairs, val: (STAIR_FINISH_OPTIONS.find(s => s.id === az.stairFinish)?.pricePerStep ?? 0) * az.stairFlights * az.stairStepsPerFlight + (az.stairRailing ? az.stairFlights * az.stairWidth * 12000 : 0) + (az.stairAntiSlip ? az.stairFlights * az.stairStepsPerFlight * 850 : 0) },
