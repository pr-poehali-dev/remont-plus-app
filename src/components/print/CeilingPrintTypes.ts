...
export interface CeilingPrintState extends ExportConfirmData {
  configs: CeilingConfig[];
  markupPct: number;
  totalSum: number;
  docNum: string;
  date: string;
...
export const CEILING_PRINT_STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ceil-block { break-inside: avoid; margin-bottom: 18px; border: 1px solid #ccc; border-radius: 4px; padding: 10px 12px; }
  .ceil-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px; }
  .ceil-num { font-size: 9pt; font-weight: 700; color: #555; }
  .ceil-title { font-size: 10pt; font-weight: 700; }
  .ceil-specs { font-size: 8.5pt; }
  .ceil-specs table { font-size: 8.5pt; }
  .ceil-specs td { padding: 2px 4px; border: none; border-bottom: 1px solid #eee; }
  .ceil-specs td:first-child { color: #555; width: 160px; }
  .ceil-specs td:last-child { font-weight: 600; }
  .ceil-price { margin-top: 8px; display: flex; justify-content: flex-end; gap: 16px; font-size: 9pt; }
  .ceil-price .total { font-weight: 700; font-size: 10.5pt; }
