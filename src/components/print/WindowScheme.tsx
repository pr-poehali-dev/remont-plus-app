...
export default function WindowScheme({ cfg, idx }: { cfg: WindowConfig; idx: number }) {
  ...
  const showTransom = cfg.hasTransom && cfg.constructionType !== "transom";
  ...
  const FRAME_W = 160;
  const MAIN_H = isStandaloneTransom ? 50 : 130;
  const TRANSOM_H = showTransom ? Math.round(MAIN_H * (cfg.transomHeight / cfg.height) * 0.6) : 0;
  const TOTAL_FRAME_H = MAIN_H + TRANSOM_H;
  const svgW = LEFT_OFF + FRAME_W + RIGHT_OFF;
  const svgH = TOP_OFF + TOTAL_FRAME_H + BOT_OFF;
  ...
  const mainFrameY = fy + TRANSOM_H;
  const displayHeight = showTransom ? cfg.height + cfg.transomHeight : cfg.height;
  ...
  return (
    <svg width={svgW} height={svgH} xmlns="http://www.w3.org/2000/svg" style={{ fontFamily: "Arial, sans-serif", overflow: "visible" }}>
      ...
      {showTransom && (
        <>
          <rect x={fx} y={fy} width={FRAME_W} height={TRANSOM_H} fill="#f0f4f8" stroke="#1a1a1a" strokeWidth={3} />
          <rect x={fx + FRAME_T} y={fy + FRAME_T} width={FRAME_W - FRAME_T * 2} height={TRANSOM_H - FRAME_T} fill="none" stroke="#555" strokeWidth={0.8} />
          {openSymbol(fx + FRAME_T + 4, fy + FRAME_T + 3, FRAME_W - FRAME_T * 2 - 8, TRANSOM_H - FRAME_T - 6, cfg.transomOpeningType)}
          <text x={fx + FRAME_W / 2} y={fy + TRANSOM_H / 2 + 3} textAnchor="middle" fontSize={7} fill="#888">фрамуга</text>
        </>
      )}

      <rect x={fx} y={mainFrameY} width={FRAME_W} height={MAIN_H} fill="#f0f4f8" stroke="#1a1a1a" strokeWidth={3} />
      <rect x={fx + FRAME_T} y={mainFrameY + FRAME_T} width={FRAME_W - FRAME_T * 2} height={MAIN_H - FRAME_T * 2} fill="none" stroke="#555" strokeWidth={0.8} />
      <rect x={fx - 4} y={mainFrameY + MAIN_H} width={FRAME_W + 8} height={6} fill="#ccc" stroke="#888" strokeWidth={0.7} rx={1} />

      {Array.from({ length: sashes }).map((_, i) => {
        const innerW = FRAME_W - FRAME_T * 2;
        const innerH = MAIN_H - FRAME_T * 2;
        const sw_inner = innerW / sashes;
        const isx = fx + FRAME_T + i * sw_inner;
        const isy = mainFrameY + FRAME_T;
        const sashPad = 4;
      ...
      <line
        x1={fx} y1={mainFrameY + MAIN_H + 16}
        x2={fx + FRAME_W} y2={mainFrameY + MAIN_H + 16}
        stroke="#444" strokeWidth={0.8}
        markerEnd={`url(#arr-${idx})`} markerStart={`url(#arrL-${idx})`}
      />
      <line x1={fx} y1={mainFrameY + MAIN_H + 12} x2={fx} y2={mainFrameY + MAIN_H + 20} stroke="#444" strokeWidth={0.8} />
      <line x1={fx + FRAME_W} y1={mainFrameY + MAIN_H + 12} x2={fx + FRAME_W} y2={mainFrameY + MAIN_H + 20} stroke="#444" strokeWidth={0.8} />
      <text x={fx + FRAME_W / 2} y={mainFrameY + MAIN_H + 26} textAnchor="middle" fontSize={9} fill="#222" fontWeight="bold">
        {cfg.width} мм
      </text>

      <line
        x1={fx - 18} y1={fy}
        x2={fx - 18} y2={mainFrameY + MAIN_H}
        stroke="#444" strokeWidth={0.8}
        markerEnd={`url(#arr-${idx})`} markerStart={`url(#arrL-${idx})`}
      />
      <line x1={fx - 22} y1={fy} x2={fx - 14} y2={fy} stroke="#444" strokeWidth={0.8} />
      <line x1={fx - 22} y1={mainFrameY + MAIN_H} x2={fx - 14} y2={mainFrameY + MAIN_H} stroke="#444" strokeWidth={0.8} />
      <text
        x={fx - 26} y={fy + TOTAL_FRAME_H / 2}
        textAnchor="middle" fontSize={9} fill="#222" fontWeight="bold"
        transform={`rotate(-90, ${fx - 26}, ${fy + TOTAL_FRAME_H / 2})`}
      >
        {displayHeight} мм
      </text>
    </svg>
